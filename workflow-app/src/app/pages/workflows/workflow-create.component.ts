    ...
    <div class="page">
      ...
      <div class="page-header">
        ...
        <div>
          ...
          <p class="page-sub">Define version, activities, transitions, and tasks</p>
        </div>
      </div>
...
export class WorkflowCreateComponent implements OnInit {
  ...
  activities: ActivityDraft[] = [this.blankActivity()];
  transitions: TransitionDraft[] = [this.blankTransition()];
  tasks: TaskDraft[] = [];

  ngOnInit(): void {
    this.engine.get<Team[]>(`/api/teams`).subscribe({
      next: (t) => this.teams = t,
      error: () => {},
    });
  }
  ...
  submit(): void {
    ...
    this.engine.post(`/api/workflow/full`, {
      ...
      error: (err) => {
        this.stepError = err.error?.error ?? 'Failed to create workflow.';
